#!/usr/bin/env python3
"""Training script for drone-rover landing using MAPPO with ACMPC policies.

Two cooperative agents (drone + rover), each with a separate ACMPC policy,
trained using centralized MAPPO with a shared critic.

Usage:
    python scripts/train_mappo_acmpc.py --experiment default
    python scripts/train_mappo_acmpc.py --experiment default --resume-run run_20260101120000
"""
...
class TerminationLoggingWrapper:
    ...
    def reset(self, *args, **kwargs):
        ...
        # Seed MAPPO_MPC's mpc_state cache from reset info
                ...
                for uid in self._agent.possible_agents:
                    if uid in infos["mpc_state"]:
                        self._agent._current_mpc_state[uid] = torch.as_tensor(
                            infos["mpc_state"][uid], dtype=torch.float32,
    ...
    def _log_events(self):
        ...
        track_data() appends to SKRL's tracking_data dict, which gets written
        and cleared by write_tracking_data() inside post_interaction().
        Since log_interval (5000) is a multiple of write_interval (100),
        both fire on the same step: our data is written before being cleared.
        """
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train MAPPO ACMPC on drone-rover landing task",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    python scripts/train_mappo_acmpc.py --experiment default
    python scripts/train_mappo_acmpc.py --experiment default --resume-run run_20260101120000
    python scripts/train_mappo_acmpc.py --experiment default --resume-run run_20260101120000 --curriculum-level 2
        """,
    )
