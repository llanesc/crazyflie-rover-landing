"""Mixed launch: CrazySim drone + real X3 rover.

Drone agent and rover agent both run on PC.
Rover subscribes to real X3's /rover/odom and publishes /cmd_vel over the network.
"""
...
def generate_launch_description():
    ...
    if 'meshes/mecanum/' in rover_urdf:
        rover_urdf = rover_urdf.replace('meshes/mecanum/base_link.STL', 'meshes/base_link_X3_bracket_only.STL')
    ...
    return LaunchDescription([
        ...
        # Static identity transform: world → map (so both drone and rover are in same tree)
        Node(
            package='tf2_ros',
        ...
        # UDP ↔ ROS2 bridge for CrazySim rover (forwards /x3/odom for mirror mode)
        Node(
        ...
        # Main executor: mission_manager + drone_agent + rover_agent (all on PC)
        # rover_odom_topic in landing_config.yaml controls which odom topic to use
        Node(
            package=package_name,
